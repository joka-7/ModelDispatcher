"""Tests for the App Check verifier's credential-parsing helper."""
...
import pytest
from _lib.appcheck import _parse_service_account_json
...
def test_parses_raw_json() -> None:
    ...
    assert _parse_service_account_json(json.dumps(payload)) == payload
...
def test_parses_base64_encoded_json() -> None:
    ...
    assert _parse_service_account_json(encoded) == payload
...
def test_tolerates_surrounding_whitespace() -> None:
    ...
    payload = {"type": "service_account", "project_id": "demo"}
    assert _parse_service_account_json(f"  {json.dumps(payload)}\n") == payload
...
def test_neither_json_nor_base64_raises() -> None:
    ...
    with pytest.raises(ValueError):
        _parse_service_account_json("not json and not base64 either !!")
